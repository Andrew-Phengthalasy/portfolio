...
export default function Header() {
    ...
    return (
        <header
            className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${scrolled ? "bg-background/80 backdrop-blur-md border-b border-accent/10 py-3" : "py-6"}`
            }
        >
            <div
                ...
                <nav
                    ...
                    {navLinks.map((link) => (
                        <button
                            ...
                            className="font-body text-md text-muted hover:text-warm transition-colors duration-200"
                        >
                            {link.label}
                        </button>
                    ))}
            ...
            }
            {
                menuOpen && (
                    <div
                        className="md:hidden bg-background/95 backdrop-blur-md border-t border-accent/10 px-6 py-6 flex flex-col gap-4">
                        {navLinks.map((link) => (
                            <button
                                key={link.label}
                                onClick={() => scrollTo(link.href)}
